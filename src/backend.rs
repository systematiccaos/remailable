...
fn sanitize_html(html: &str) -> String {
    ...
    // Merge heading styles with any existing style attribute to avoid duplicates
    result = merge_heading_style(&result, "h1", "font-size:38px;font-weight:bold;margin:16px 0 8px 0;color:#111");
    result = merge_heading_style(&result, "h2", "font-size:32px;font-weight:bold;margin:14px 0 6px 0;color:#111");
    result = merge_heading_style(&result, "h3", "font-size:28px;font-weight:bold;margin:12px 0 6px 0;color:#111");
    result = merge_heading_style(&result, "h4", "font-size:24px;font-weight:bold;margin:10px 0 4px 0;color:#222");
    result = merge_heading_style(&result, "h5", "font-size:22px;font-weight:bold;margin:8px 0 4px 0;color:#222");
    result = merge_heading_style(&result, "h6", "font-size:20px;font-weight:bold;margin:8px 0 4px 0;color:#333");
...
fn merge_heading_style(html: &str, tag: &str, defaults: &str) -> String {
    let re = regex::Regex::new(&format!(r"(?i)<({0})([^>]*)>", regex::escape(tag))).unwrap();
    re.replace_all(html, |caps: &regex::Captures| {
        let attrs = caps.get(2).map(|m| m.as_str()).unwrap_or("");
        if attrs.contains("style=") {
            // Merge existing style with defaults (defaults go first, existing wins)
            let existing_re = regex::Regex::new(r#"(?i)\sstyle\s*=\s*"([^"]*)"|\sstyle\s*=\s*'([^']*)'"#).unwrap();
            let mut merged = defaults.to_string();
            if let Some(caps) = existing_re.captures(attrs) {
                let existing = caps.get(1).or_else(|| caps.get(2)).map(|m| m.as_str()).unwrap_or("");
                merged = format!("{};{}", defaults, existing);
            }
            let cleaned = existing_re.replace(attrs, "");
            format!("<{} {} style=\"{}\">", &caps[1], cleaned.trim(), merged)
        } else {
            format!("<{} {} style=\"{}\">", &caps[1], attrs, defaults)
        }
    }).to_string()
}
